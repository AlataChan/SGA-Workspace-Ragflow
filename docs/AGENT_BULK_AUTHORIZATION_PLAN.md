# Agent 管理页：批量授权方案（v1.1）

> **依赖说明**：本方案依赖 `docs/MDM_DEPARTMENT_SYNC_IMPLEMENTATION.md` 中的 MDM 部门同步功能。  
> 实施顺序：先完成 MDM 部门同步（阶段一）→ 再实施本批量授权方案。
...
> `POST /api/admin/users/bulk/grant-all-agents`（createMany + skipDuplicates）。  
> 本方案希望在 Agent 管理页提供对称能力：**把一个 Agent 批量授权给一批用户**。
...
- 选择一个 Agent
- 选择目标用户范围（部门 / 全公司 / 指定用户），其中**部门范围以 MDM 同步的组织结构为准**
...
本项目已规划“部门从 MDM 同步并支持树形层级”的能力（见：`docs/MDM_DEPARTMENT_SYNC_IMPLEMENTATION.md`）。  
因此，Agent 批量授权在“按部门授权”模式下需要做如下适配：

- **部门来源**：以本系统的 `Department` 表为准，但其数据来自 MDM 同步结果（组织结构完全照搬 MDM）。
...
- **不回传 MDM**：本方案所有授权行为仅影响本系统的 `user_agent_permissions`，不调用 MDM 写接口。
...
async function getDepartmentWithDescendants(
  ...
  if (!targetDept?.mdmLcode) {
    return [departmentId]; // 无 lcode（本地部门），仅返回自身
  }
 ...
 * 批量获取多个部门的所有后代
 */
async function getMultipleDepartmentsWithDescendants(
  ...
  for (const deptId of departmentIds) {
    const descendants = await getDepartmentWithDescendants(deptId, companyId);
    descendants.forEach(id => allDeptIds.add(id));
  }
...
**性能估算**：
- 部门总数：500 个，层级 10-15 级
- 单次前缀查询：< 10ms（有索引）
- 相比递归 CTE：性能提升 10-50 倍

**前端部门选择**：使用树接口 `GET /api/admin/departments/tree` 以避免在 UI 侧重复建树逻辑。
...
```mermaid
graph TD
    subgraph Phase1[阶段一：MDM 部门同步]
        A1[1.1 扩展 Department 模型] --> A2[1.2 实现 MDM 部门同步服务]
        A2 --> A3[1.3 部门管理页-同步入口]
        A3 --> A4[1.4 部门树接口与组件]
    end
    
    subgraph Phase2[阶段二：用户部门同步]
        B1[2.1 扩展 User 模型] --> B2[2.2 实现用户部门同步]
        B2 --> B3[2.3 用户管理页-部门覆盖功能]
    end
    
    subgraph Phase3[阶段三：Agent 批量授权]
        C1[3.1 新增撤销黑名单表] --> C2[3.2 实现批量授权 API]
        C2 --> C3[3.3 Agent 管理页-批量授权弹窗]
        C3 --> C4[3.4 用户管理页-撤销权限增强]
    end
