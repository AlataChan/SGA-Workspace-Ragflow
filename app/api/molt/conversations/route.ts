...
export async function GET(request: NextRequest) {
  try {
    ...
    const payload = await createMoltClient(request, agentId).listConversations(
      agentId,
      user.userId,
      {
    ...
    console.error("[Molt Conversations] Error:", error)
    return jsonError(500, "INTERNAL_ERROR", "Molt conversation list failed")
  }
}
