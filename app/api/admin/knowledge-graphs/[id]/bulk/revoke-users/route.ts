/**
 * Knowledge Graph 批量撤销（写 revocation 黑名单）API
...
async function countAlreadyRevoked(knowledgeGraphId: string, userIds: string[]) {
  if (userIds.length === 0) return 0
  const batches = chunkArray(userIds, 2000)
  let total = 0
  for (const batch of batches) {
    total += await prisma.userKnowledgeGraphPermissionRevocation.count({
      where: {
        knowledgeGraphId,
        userId: { in: batch },
...
async function countExistingExplicit(knowledgeGraphId: string, userIds: string[]) {
  if (userIds.length === 0) return 0
  const batches = chunkArray(userIds, 2000)
  let total = 0
  for (const batch of batches) {
    total += await prisma.userKnowledgeGraphPermission.count({
      where: {
        knowledgeGraphId,
        userId: { in: batch },
      },
  ...
  try {
    ...
    if (input.dryRun) {
      const [alreadyRevoked, explicitCount] = await Promise.all([
        countAlreadyRevoked(knowledgeGraphId, targetUserIds),
        countExistingExplicit(knowledgeGraphId, targetUserIds),
      ])
    ...
    for (const batch of batches) {
      const [deleted, created, updated] = await prisma.$transaction([
        prisma.userKnowledgeGraphPermission.deleteMany({
          where: { knowledgeGraphId, userId: { in: batch } },
        }),
        prisma.userKnowledgeGraphPermissionRevocation.createMany({
    ...
    await writeAuditEvent({
      ...
      eventType: "PERM_KNOWLEDGE_GRAPH_BULK_REVOKE",
      result: "SUCCESS",
